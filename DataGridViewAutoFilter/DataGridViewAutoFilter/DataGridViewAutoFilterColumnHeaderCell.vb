...
Public Class DataGridViewAutoFilterColumnHeaderCell
    ...
    Public WithEvents PopulateFilterThread As New BackgroundWorker
    ...
    ''' the owning column. 
    ''' </summary>
    Public selectedFilterValue As String = String.Empty
    ...
    Public FilterPopulated As Boolean = False
    Public DropDownFilled As Boolean = False
    Public CurrPropertiesLoadIndex As Int64 = 0
    Public CurrNonDupPropertiesLoadCount As Int64 = 0
    Public CurrDupPropertiesLoadCount As Int64 = 0
    Public CurrItemsLoadIndex As Int64 = 0
    ...
    Private Sub VerifyDataSource()
        ...
        If Me.DataGridView Is Nothing OrElse
...
#End Region 'DataGridView events
    ...
    Protected Overrides Sub Paint(
        ...
        If Application.RenderWithVisualStyles Then
            ...
            End If
            buttonBounds.X = buttonBounds.X - 2
    ...
    Public Sub SimulateMouseDown(ByVal e As DataGridViewCellMouseEventArgs)
        OnMouseDown(e)
    End Sub
    ...
    Protected Overrides Sub OnMouseDown(ByVal e As DataGridViewCellMouseEventArgs)
        ...
        ElseIf AutomaticSortingEnabled AndAlso
            ...
            DataGridViewSelectionMode.ColumnHeaderSelect AndAlso e.Button = MouseButtons.Left Then
...
#Region "filtering: PopulateFilters, FilterWithoutCurrentColumn, UpdateFilter, RemoveFilter, AvoidNewRowWhenFiltering, GetFilterStatus"
    ...
    ''' </summary>
    Public Sub PopulateFilters(Optional ByVal Async As Boolean = False)
        If FilterPopulated = False Then
            If Not PopulateFilterThread.IsBusy Then
                CurrItemsLoadIndex = 0
                CurrPropertiesLoadIndex = 0
                CurrNonDupPropertiesLoadCount = 0
                CurrDupPropertiesLoadCount = 0

                If PopulateFilterThread.IsBusy = False Then
                    PopulateFilterThread.RunWorkerAsync()
                End If

                If Async = False Then
                    Do While PopulateFilterThread.IsBusy
    ...
    Private Sub PopulateFilters_DoWork() Handles PopulateFilterThread.DoWork
        ...
        For Each item In data
            ...
            If ictd IsNot Nothing Then
                ...
                For Each prop As PropertyDescriptor In properties
                    ...
                    If (String.Compare(Me.OwningColumn.DataPropertyName,
                        prop.Name, True,
            ...
            If Not ContainsLoop(list, value) Then
                CurrNonDupPropertiesLoadCount += 1
                list.Add(value)
            Else
                CurrDupPropertiesLoadCount += 1
            End If
        Next item
        ...
        For ind As Integer = 0 To list.Count - 1
            ...
            formattedValue = CStr(GetFormattedValue(list(ind), -1, style,
            ...
            ElseIf Not filters.Contains(formattedValue) Then
                ...
                ' dictionary.
                filters.Add(formattedValue, list(ind).ToString())
            End If
        Next
    ...
    Private Shared Function ContainsLoop(ByVal list As List(Of Object), ByVal value As Object) As Boolean
    ...
    ''' </summary>
    Public Sub UpdateFilter(Optional ByVal FilterValue As String = "")
        ' Continue only if the selection has changed.
        If FilterValue = "" Then
            If dropDownListBox.Visible Then
        ...
        Else
            ' Store the new selection value from optional argument
            ...
            Select Case selectedFilterValue
                ...
                Case "(NonBlanks)"
                    newColumnFilter = String.Format(
                        "LEN(ISNULL(CONVERT([{0}],'System.String'),''))>0", columnProperty)
                Case Else
                    newColumnFilter = String.Format("[{0}]='{1}'", columnProperty,
                        CStr(filters(selectedFilterValue)).Replace("'", "''"))
            End Select
        Catch ex As Exception
            'Es wurde ein Eintrag ausgewählt, welcher nicht in der Auflistung verfügbar ist = Keine Einträge vorhanden
        End Try
    ...
    Public Shared Function GetFilterStatus(
        ByVal dataGridView As ExtendedDataGridView) As String
